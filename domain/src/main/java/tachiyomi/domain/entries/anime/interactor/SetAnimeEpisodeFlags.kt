...
class SetAnimeEpisodeFlags(
    ...
    suspend fun awaitSetEpisodeGridSize(anime: Anime, size: Int): Boolean {
        return animeRepository.updateAnime(
            AnimeUpdate(
                id = anime.id,
                episodeFlags = anime.episodeFlags.setFlag(
                    (size shl Anime.EPISODE_GRID_DISPLAY_SIZE_BIT_OFFSET).toLong(),
                    Anime.EPISODE_GRID_DISPLAY_SIZE_MASK,
                ),
            ),
        )
    }
