            ...
            when (page) {
                ...
                2 -> {
                    DisplayPage(
                        ...
                        gridSize = anime?.episodeGridSize ?: 0,
                        onGridSizeChanged = onDisplayGridSizeChanged,
...
private fun ColumnScope.DisplayPage(
    ...
    listOf(
        ...
        AYMR.strings.show_episode_grid to Anime.EPISODE_DISPLAY_GRID,
    ).map { (titleRes, mode) ->
        RadioItem(
            label = stringResource(titleRes),
    ...
    if (displayMode == Anime.EPISODE_DISPLAY_GRID) {
        SliderItem(
            value = gridSize,
            valueRange = 0..5,
            label = stringResource(MR.strings.pref_library_columns),
            valueText = if (gridSize > 0) {
                gridSize.toString()
            } else {
